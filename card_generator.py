...
import os, uuid, threading, shutil, time
...
api_key = os.environ.get("OPENAI_API_KEY")  # Renderの環境変数を取得
if not api_key:
...
# ==== 一時ディレクトリ自動削除関数 ====
def cleanup_temp(dir_path, delay=300):
    def _delete():
        time.sleep(delay)
...
# Windows の場合
font_path = r"static\Fonts\YuGothR.ttc"  # フォントファイルの絶対パス
font_path_name = r"static\Fonts\YuGothB.ttc" 
...
flame_path_base = r"static\Card_frame"
...
# === 3️⃣ カード合成 ===
def compose_card(nickname_jp, char_img_path, frame_color, card_color, name, boss_explanation, output_path):
    # --- 既存のカード背景画像を読み込む ---
    
    if card_color=="black":
        if frame_color=="white":
            background_path = os.path.join(flame_path_base, "black_common.png")
        elif frame_color=="blue":
            background_path = os.path.join(flame_path_base, "black_rare.png")
        elif frame_color=="yellow":
            background_path = os.path.join(flame_path_base, "black_srare.png")
            
            
    if card_color=="red":
        if frame_color=="white":
            background_path = os.path.join(flame_path_base, "red_common.png")
        elif frame_color=="blue":
            background_path = os.path.join(flame_path_base, "red_rare.png")
        elif frame_color=="yellow":
            background_path = os.path.join(flame_path_base, "red_srare.png")
            
            
    if card_color=="green":
        if frame_color=="white":
            background_path = os.path.join(flame_path_base, "green_common.png")
        elif frame_color=="blue":
            background_path = os.path.join(flame_path_base, "green_rare.png")
        elif frame_color=="yellow":
            background_path = os.path.join(flame_path_base, "green_srare.png")
    
    
    if card_color=="lightblue":
        if frame_color=="white":
            background_path = os.path.join(flame_path_base, "skyblue_common.png")
        elif frame_color=="blue":
            background_path = os.path.join(flame_path_base, "skyblue_rare.png")
        elif frame_color=="yellow":
            background_path = os.path.join(flame_path_base, "skyblue_srare.png")
            
            
    if card_color=="sienna":
        if frame_color=="white":
            background_path = os.path.join(flame_path_base, "sienna_common.png")
        elif frame_color=="blue":
            background_path = os.path.join(flame_path_base, "sienna_rare.png")
        elif frame_color=="yellow":
            background_path = os.path.join(flame_path_base, "sienna_srare.png")
            
    if card_color=="plum":
        if frame_color=="white":
            background_path = os.path.join(flame_path_base, "plum_common.png")
        elif frame_color=="blue":
            background_path = os.path.join(flame_path_base, "plum_rare.png")
        elif frame_color=="yellow":
            background_path = os.path.join(flame_path_base, "plum_srare.png")
    ...
    # --- キャラクター画像 ---
    char_img = Image.open(char_img_path).resize((885, 890)).convert("RGBA")

# αチャンネルをマスクに使用
    mask = char_img.split()[3]  # Aチャンネルだけ取り出す

    background.paste(char_img, (83, 364), mask)
...
def add_glitter_effect(image, count=150):
    ...
    return image
# ここに元の generate_nickname, compose_card の関数定義を入れるか
# card_maker.py にまとめて import してもOK
#from card_maker import generate_nickname, compose_card

def generate_boss_explanation(nickname_jp, rarity):
    """
    ボス説明文をAIに生成させる
    ...
    # プロンプト例
    prompt = (
        f"あなたはダークファンタジーのゲームの世界観の作家です。\n"
        f"キャラクターの二つ名は「{nickname_jp}」です。\n"
        f"レアリティは「{rarity}」です。\n"
        f"このボスキャラクターの短い説明文（50〜80文字程度）を作ってください。\n"
        f"注意: 二つ名を文章内で直接書かず、自然な文章として表現してください。"
    )
    ...
    response = client.chat.completions.create(
        model="gpt-4o-mini",
        messages=[
            {"role": "system", "content": "あなたはダークファンタジーのゲーム作家です。"},
...
flame_path_base = "static/Card_frame"
#test_char_path = os.path.join(flame_path_base, "test.png")
...
def generate_card_route():
    ...
    #AI画像生成
    ai_image_path = os.path.join(user_dir, "ai_character.png")
    generate_character_image(creature_en, adj_en, place_en, rarity, filename=ai_image_path)


    # 一時出力パス
    output_path = os.path.join(user_dir, "generated_card.png")

    # 説明文
    boss_explanation = generate_boss_explanation(nickname_jp, rarity)

    # カード合成
    compose_card(nickname_jp, ai_image_path, frame_color, card_color, name, boss_explanation, output_path)


    # 自動削除予約（5分後）
    cleanup_temp(user_dir, delay=300)
